class Car < ActiveRecord::Base
      ...
      if (cars_for_date(flight_date).uniq.size < Car.all.size)
        cars_with_no_flights_for_date(flight_date).each do |car|
          if cars_with_multiple_flights_for_date(flight_date).present?
            flight_to_update = Flight.flights_for_date(flight_date).detect{|fl| fl.car == cars_with_multiple_flights_for_date(flight_date).last}
            flight_to_update.update_attribute(:car_id, car.id)
          end
        end
      end
    end
  ...
  def self.cars_for_date(date)
    Flight.flights_for_date(date).map(&:car)
  end

  def self.cars_with_no_flights_for_date(date)
    (Car.all.to_a - cars_for_date(date).uniq).reject(&:shuttle)
  end
